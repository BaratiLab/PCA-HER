...
from baselines.der.util import (
    import_function, store_args, flatten_grads, transitions_in_episode_batch, convert_episode_to_batch_major)
from baselines.der.normalizer import Normalizer
from baselines.der.replay_buffer import ReplayBuffer
from baselines.der.hv_buffer import HVBuffer
...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
                 ...
                 sample_transitions, gamma, reuse=False, **kwargs): # successful_only, evar, shape, stdevs,
        ...
        # HV Buffer
        hv_buffer_size = (self.hv_buffer_size // self.rollout_batch_size) * self.rollout_batch_size
        self.HV_BUFFER = HVBuffer(buffer_shapes, hv_buffer_size, self.hv_successful_only, 
                                  self.hv_evar, self.hv_shape, self.hv_stdevs, self.hv_rung_enabled,
                                  self.hv_collect_outliers)
    ...
    def get_actions(self, o, ag, g, noise_eps=0., random_eps=0., use_target_net=False,
        ...
        eps = np.random.binomial(1, random_eps, u.shape[0]).reshape(-1, 1) * (self._random_action(u.shape[0]) - u)  # eps-greedy
        u += eps
        if u.shape[0] == 1:
            u = u[0]
        u = u.copy()
    ...
    def store_episode(self, episode_batch, update_stats=True):
        ...
        self.total_episode_count += 1 # ADDED BY JUAN 10/17/2019
    ...
    def init_HV_buffer(self,HVDataFile, update_stats = True):
        ...
        logger.debug("HV Buffer buffer size currently ", self.HV_BUFFER.get_current_size()) #print out the demonstration buffer size
    ...
    def update_HV_buffer(self):
        self.HV_BUFFER.update_buffer(self.buffer.buffers)
    ...
    def _grads(self):
        ...
        critic_loss, actor_loss, Q_grad, pi_grad = self.sess.run([
            self.Q_loss_tf, # Critic loss
            self.main.Q_pi_tf, # Actor loss
            self.Q_grad_tf, # Critic gradient
            self.pi_grad_tf # Actor gradient
        ])
        #self.critic_loss_list.append(critic_loss)   # For troubleshooting ONLY
        #self.actor_loss_list.append(actor_loss)     # For troubleshooting ONLY
    ...
    def sample_batch(self):
        ...
        #print('DER Samplig: ',not self.bc_loss,self.HV_BUFFER.get_current_size() > self.hv_batch_size)
        ...
        elif (not self.bc_loss and self.HV_BUFFER.get_current_size() > self.hv_batch_size):# or self.hv_buffer_enabled:
            if  (self.hv_buffer_shutoff > 0 and self.hv_buffer_shutoff >= self.total_episode_count) or (self.hv_buffer_shutoff == 0):
                # ADDED BY JUAN 10/17/2019
                transitions = self.buffer.sample(self.batch_size - self.hv_batch_size)
                transitions_hv = self.HV_BUFFER.sample(self.hv_batch_size)
                for k, values in transitions_hv.items():
                    rolloutV = transitions[k].tolist()
                    for v in values:
                        rolloutV.append(v.tolist())
                    transitions[k] = np.array(rolloutV)
            else:
                #print('HVB OFFLINE',self.hv_buffer_shutoff,self.total_episode_count)
                transitions = normal_sampling()
        else:
            transitions = normal_sampling() #otherwise only sample from primary buffer
    ...
    def save_buffer(self, save_path, er = True, hv = True):
        if er: tf_util.save_buffer(save_path, self.buffer.buffers, as_pkl = True)
        if hv: tf_util.save_buffer(save_path, self.HV_BUFFER.buffers, as_pkl = True, filename = "hv_buffer.pkl")
